...
Vector2f popup_plane::direction_hit_boundary(Vector2f pt,Vector2f direc)
{
	...
	if (lambd>=0){  // up direction
	    ...
	    if ((0<= (int)hit_pt(0)) && ((int)hit_pt(0)<= width-1))
		return hit_pt;
	}
    }
    if (direc(1)>0){  // hit bottom edge      
	lambd=(height-1.0-pt(1))/direc(1);
	...
	lambd=(width-1.0-pt(0))/direc(0);
	if (lambd>=0){  // right direction
	    Vector2f hit_pt=pt+lambd*direc;
	    if ((0<= (int)hit_pt(1)) && ((int)hit_pt(1)<= height-1))
		return hit_pt;
	}
    }    
...
tuple<MatrixXf, MatrixXf, VectorXf> popup_plane::edge_get_polygons(cv::Mat &lsd_edges,const cv::Mat &label_map)
{
	...
	std::vector<int> not_vertical_ind;
	{
		...
		continue;
	      ...
	      if ((abs(abs(line_angle)-90)>pre_vertical_thre))
...
// 	cout<<"close_line_ind size "<<close_line_ind.size()<<endl;
	...
	// Step3: merge two short edges if they are very parallel, and very close (basically original from the same line segments)  // example:   ----- ----
	{
	    ...
	    while ((can_force_merge) && (counter<100)){
			...
			for (int seg2=seg1+1;seg2<line_number;seg2++){
			      float diff=abs(all_angles(seg1)-all_angles(seg2));
			    ...
			    for (int seg2=seg1+1;seg2<line_number;seg2++){
				    ...
				    if (angle_diff<pre_proj_angle_thre){   // if nearly parallel
					    ...
					    if ( (dist_1_bg_to_2<pre_proj_dist_thre) && (dist_1_ed_to_2<pre_proj_dist_thre) && \
						    ...
						    float covering_1_to_2=abs(proj_1_bg_to_2-proj_1_ed_to_2);
						    float covering_2_to_1=abs(proj_2_bg_to_1-proj_2_ed_to_1);
...
// 	cout<<"optimized_line_segs_mat row "<<optimized_line_segs_mat.rows<<endl;
	...
	//           ----
	{
	    ...
	    while ((can_force_merge) && (counter<100)){
		    ...
		    for (int seg=0;seg<line_number-1;seg++) {
			    float diff=abs(all_angles(seg)-all_angles(seg+1));
			    float angle_diff=min(diff,180-diff);
        ...
        // extend first and last seg to hit the boundary.
	{
	    ...
	    Vector2i new_start_pt=(direction_hit_boundary(final_long_segs.row(0).tail(2),final_long_segs.row(0).head(2)-final_long_segs.row(0).tail(2))).cast<int>();
	    Vector2i new_end_pt=(direction_hit_boundary(final_long_segs.bottomRows<1>().head(2),final_long_segs.bottomRows<1>().tail(2)-final_long_segs.bottomRows<1>().head(2))).cast<int>();
