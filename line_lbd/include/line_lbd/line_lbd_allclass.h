...
public:
    ...
    void detect_raw_lines(const cv::Mat& gray_img, std::vector<std::vector< KeyLine>>& keyline_octaves);  // without line thresholding        
    void detect_raw_lines(const cv::Mat& gray_img, cv::Mat& linesmat_out, bool downsample_img=false);  /* output is n*4 mat. x1 y1 x2 y2 */
    ...
    /* detect edges and compute edge descriptors */
    void detect_descrip_lines_downsample(const cv::Mat& gray_img, cv::Mat& linesmat_out, cv::Mat& line_descrips);
...
void mat_to_keylines(const cv::Mat& linesmat_src, std::vector< KeyLine>& keylines_out, 
			  int raw_img_width, int raw_img_height, float raw_length_threshold=0, float scaling=1,
