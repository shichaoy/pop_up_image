...
void line_lbd_detect::detect_descrip_lines(const cv::Mat& gray_img, cv::Mat& lines_mat, Mat& line_descrips)
{
  ...
  lbd->compute( gray_img, keylines, descrip_raw);
  
  /* select keylines from first octave and their descriptors */
  std::vector<KeyLine> lbd_octave;  // keylines
...
//   std::cout<<"marker 1.1  finish detect raw lines"<<std::endl;
  ...
  line_descrips = cv::Mat().clone();
  for ( int i = 0; i < (int) keylines.size(); i++ )
      if( keylines[i].octave == 0  )  // only first octave?
	 if (keylines[i].lineLength>line_length_thres){
	    keylines_out.push_back( keylines[i] );
	    line_descrips.push_back( descrip_raw.row( i ) );
	 }  
}
...
// 	std::cout<<"kl.lineLength "<<kl.angle<<std::endl;
	if (kl.lineLength*octaveScale>line_length_thres)
	{
	      ...
	      {
		  std::swap(kl.startPointX,kl.endPointX);  std::swap(kl.startPointY,kl.endPointY);
		  std::swap(kl.sPointInOctaveX,kl.ePointInOctaveX); std::swap(kl.sPointInOctaveY,kl.ePointInOctaveY);
